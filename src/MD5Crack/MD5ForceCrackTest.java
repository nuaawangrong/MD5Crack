...
public class MD5ForceCrackTest {

    public static int len;
    public static int testValueAmout = 1000;
    public static boolean onlyNumber = false;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i=0;i<lenArr.length;i++) {
            for(int j=0;j<testValueAmoutArr.length;j++) {
                ...
                System.out.format("len = %d, testValueAmout = %d\n",len,testValueAmout);
                forceCrackTest();
            }
        }
    }

    public static void forceCrackTest() throws Exception {
        int crackedCnt = 0;
        int uncrackedCnt = 0;
        ...
        if(onlyNumber) {
            ...
            fileNameTestLog = "forceCrackTestLog_number_"+len+"位_随机"+testValueAmout+"个_Linux.txt";
        } else {
            fileName = "test_lowercaseLetterAndNumber_"+len+"位_"+testValueAmout+".txt";
            fileNameTestLog = "forceCrackTestLog_lowercaseLetterAndNumber_"+len+"位_随机"+testValueAmout+"个_Linux.txt";
        }
        ...
        for (int i=0;i<testValueAmout;i++) {
            ...
            //暴力破解分析
            ForceCrack forceCrack = new ForceCrack();                    //暴力破解
            crackSuccess = forceCrack.crack(MD5Value[i][0],MD5Value[i][1], len, onlyNumber);
            System.out.println("暴力破解: " + i + "/" + testValueAmout);
            writer.append("暴力破解: " + i + "/" + testValueAmout+"\n");
